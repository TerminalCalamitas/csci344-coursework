...
class PostListEndpoint(Resource):
    ...
    def get(self):
        ...
        if limit > 50:
            return Response(
                json.dumps({"message": "limit must be less than 50"}),
                mimetype="application/json",
                status=400,
            )
        ...
        posts = Post.query.filter(Post.user_id.in_(ids_for_me_and_my_friends)).limit(
            limit
        )
...
class PostDetailEndpoint(Resource):
    ...
    def patch(self, id):
        ...
        return Response(
            json.dumps(new_post.to_dict()), mimetype="application/json", status=200
        )
