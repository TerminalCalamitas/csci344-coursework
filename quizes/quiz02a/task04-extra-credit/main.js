...
function showMatchingTracks() {
  ...
  // filter artists that match the search search term
  let matchArtists = tracks.filter(doesArtistMatch);

  //filter tracks that match the search term add any new matches to the array
  let matchTracks = tracks.filter(doesTrackMatch);

  //filter albums that match the search term
  let matchAlbums = tracks.filter(doesAlbumMatch);
  
  //add any artist matches to the array
  for (const track of matchArtists) {
      matches.push(track);
  }
  // add any new track matches to the array
  for (const track of matchTracks) {
    if (!matches.includes(track)) {
      matches.push(track);
    }
  }
  // add any new album matches to the array
  for (const track of matchAlbums) {
    if (!matches.includes(track)) {
      matches.push(track);
    }
  }
